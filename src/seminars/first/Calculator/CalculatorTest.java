...
public class CalculatorTest {
    public static void main(String[] args) {
        //1. Проверка метода с целыми числами:
...
//2. Проверка метода с нулевой скидкой:
        assertThat(Calculator.calculatingDiscount(100, 0)).isEqualTo(100);
//3. Проверка метода при 100% скидке:
        assertThat(Calculator.calculatingDiscount(200, 100)).isEqualTo(0);
//4. Проверка метода при отрицательной скидке:
        assertThatThrownBy(() -> Calculator.calculatingDiscount(100, -10))
                .isInstanceOf(ArithmeticException.class);
//5. Проверка метода при скидке 200%:
        assertThatThrownBy(() -> Calculator.calculatingDiscount(200, 200))
                .isInstanceOf(ArithmeticException.class);
//6. Проверка метода при отрицательной цене:
        assertThatThrownBy(() -> Calculator.calculatingDiscount(-100, 20))
                .isInstanceOf(ArithmeticException.class);
//7. Проверка наличия сообщения об ошибке в методе:
